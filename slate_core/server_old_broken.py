...
async def periodic_discovery_health_check():
    ...
    global discovery_running  # Add global declaration
...
async def startup_event():
    ...
    # 🚀 START WORLD-CLASS DISCOVERY SYSTEM
    # Replacing broken swarm parameter tuning with proper quantitative strategies
    try:
        from slate_core.world_class_server_integration import start_world_class_discovery_on_startup
        world_class_result = await start_world_class_discovery_on_startup()
        logger.info("✅ WORLD-CLASS DISCOVERY SYSTEM STARTED")
        logger.info("   • Market regime awareness enabled")
        ...
        logger.warning("⚠️  Falling back to legacy discovery system")

    # Legacy startup coordinator (only if world-class fails)
    if STARTUP_COORDINATOR_AVAILABLE and False:  # Disabled to force world-class system
        try:
